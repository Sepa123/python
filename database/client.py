...
### update estado entrega de nota venta
    ...
    def insert_beetrack_data_ruta_transyanez(self, data):
            with self.conn.cursor() as cur:
                ...
                VALUES( %(route_id)s, %(identifier)s, %(guide)s, %(Cliente)s, '', %(Servicio)s, %(Región de despacho)s, null, %(estimated_at)s, null, %(substatus)s, %(driver)s, null, %(contact_identifier)s, %(contact_name)s,  %(contact_address)s, %(contact_phone)s, %(contact_email)s, '', 0, 0, %(fechahr)s, null, %(contact_email)s, %(driver)s, %(driver)s, %(fechaentrega)s, null,  %(comuna)s, null,  %(volumen)s, %(bultos)s, null, %(factura)s, %(oc)s, %(ruta)s, %(tienda)s, null, %(codigo)s, %(observacion)s);
