            ...
            select ROW_NUMBER() OVER (ORDER BY "ID. RUTA") as "N°", 
                ...
                from(
                    select 
                            ...
                            trb.identificador_ruta as "ID. RUTA", 
                            initcap(trb.usuario_movil) as "DRIVER", 
                            trb.identificador as "PATENTE", 
                            coalesce(trb.region_de_despacho,(select tcr.region_beetrack from public.ti_comuna_region tcr  
                                                                where unaccent(lower(tcr.region)) = unaccent(lower(trb.lugar_de_trabajo))
                                                                group by tcr.region_beetrack )) as "REGION",
                            coalesce (trb.distancia_km,0) as "Km. Ruta",
                            --count(*) as t-ped,
                            (select count(*) from beetrack.ruta_transyanez aux where aux.identificador_ruta = trb.identificador_ruta) as "T-PED",
                            --aqui iban las tiendas---
                            -- Rurales
                            -- Hora Porcentaje Entrega
                            --11:00
                            (select count(*) from beetrack.ruta_transyanez h1100
                            where h1100.identificador_ruta=trb.identificador_ruta and to_char(h1100.fecha_llegada,'hh24mi')<='1100') as "C11" ,
                            round((select count(*) 
                            from beetrack.ruta_transyanez h11002 
                            where to_char(h11002.fecha_llegada,'hh24mi')<='1100'
                                and h11002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h11003 
                            where h11003.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "(%) 11" ,
                            --13:00
                            (select count(*) from beetrack.ruta_transyanez h1300
                            where h1300.identificador_ruta=trb.identificador_ruta and to_char(h1300.fecha_llegada,'hh24mi')<='1300') as "C13",
                            round((select count(*) 
                            from beetrack.ruta_transyanez h13002 
                            where to_char(h13002.fecha_llegada,'hh24mi')<='1300'
                                and h13002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h13003 
                            where h13003.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "(%) 13",
                            --15:00
                            (select count(*) from beetrack.ruta_transyanez h1500
                            where h1500.identificador_ruta=trb.identificador_ruta and to_char(h1500.fecha_llegada,'hh24mi')<='1500') as "C15",
                            round((select count(*) 
                            from beetrack.ruta_transyanez h15002 
                            where to_char(h15002.fecha_llegada,'hh24mi')<='1500'
                                and h15002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h15003 
                            where h15003.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "(%)15",
                            --17:00
                            (select count(*) from beetrack.ruta_transyanez h1700
                            where h1700.identificador_ruta=trb.identificador_ruta and to_char(h1700.fecha_llegada,'hh24mi')<='1700') as  "C17",
                            round((select count(*) 
                            from beetrack.ruta_transyanez h17002 
                            where to_char(h17002.fecha_llegada,'hh24mi')<='1700'
                                and h17002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h17003 
                            where h17003.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "(%)17",
                            --18:00
                            (select count(*) from beetrack.ruta_transyanez h1800
                            where h1800.identificador_ruta=trb.identificador_ruta and to_char(h1800.fecha_llegada,'hh24mi')<='1800') as "C18",
                            round((select count(*) 
                            from beetrack.ruta_transyanez h18002 
                            where to_char(h18002.fecha_llegada,'hh24mi')<='1800'
                                and h18002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h18003 
                            where h18003.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "(%)18",
                            --20:00
                            (select count(*) from beetrack.ruta_transyanez h2000
                            where h2000.identificador_ruta=trb.identificador_ruta and to_char(h2000.fecha_llegada,'hh24mi')<='2000') as "C20",
                            round((select count(*) 
                            from beetrack.ruta_transyanez h20002 
                            where to_char(h20002.fecha_llegada,'hh24mi')<='2000'
                                and h20002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h20003 
                            where h20003.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "(%)20",
                            --24:00
                            ((select count(*) from beetrack.ruta_transyanez h2400
                            where h2400.identificador_ruta=trb.identificador_ruta and to_char(h2400.fecha_llegada,'hh24mi')<='2359') ||' (' ||
                            round((select count(*) 
                            from beetrack.ruta_transyanez h24002 
                            where to_char(h24002.fecha_llegada,'hh24mi')<='2359'
                                and h24002.identificador_ruta = trb.identificador_ruta)*100/(select count(*) 
                            from beetrack.ruta_transyanez h24003 
                            where h24003.identificador_ruta = trb.identificador_ruta)::numeric, 0) || '%)') as "Final_D",
                            -- Observación Ruta, Finalizada
                        
                            -- Hora Minima y Maxima
                            (select min(to_timestamp(to_char(horamin.fecha_llegada,'hh24:mi'),'hh24:mi')) from beetrack.ruta_transyanez horamin
                            where horamin.identificador_ruta = trb.identificador_ruta) as "H_INIC",
                            
                            (select max(to_timestamp(to_char(horamax.fecha_llegada,'hh24:mi'),'hh24:mi')) from beetrack.ruta_transyanez horamax
                            where horamax.identificador_ruta = trb.identificador_ruta) as "H_TERM",
                            
                            ---termina Hora Minima y Maxima
                            --- tt-ruta
                            -- PROMEDIO POR ENTREGAS
                            -- conteos
                            (select count(*) from beetrack.ruta_transyanez entt
                            where entt.identificador_ruta = trb.identificador_ruta and lower(entt.estado) in ('entregado')) as "T-ENT",
                            (select count(*) from beetrack.ruta_transyanez nentt
                            where nentt.identificador_ruta = trb.identificador_ruta and lower(nentt.estado) in ('no entregado')) as "N-ENT" ,
                            round((select count(*) from beetrack.ruta_transyanez entt
                            where entt.identificador_ruta = trb.identificador_ruta and lower(entt.estado) in ('entregado'))*100/(select count(*) from beetrack.ruta_transyanez nentt
                            where nentt.identificador_ruta = trb.identificador_ruta)::numeric, 0) as "EE"
                        from beetrack.ruta_transyanez trb
                        -- where to_char(trb.created_at,'yyyymmdd') = '20231023' -- Fecha como Variable
                        where to_char(trb.created_at,'yyyymmdd') = to_char(current_date,'yyyymmdd')
                        and identificador_ruta notnull
