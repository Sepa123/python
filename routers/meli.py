    ...
    try:
        ...
        for datos in data_supervisor.datos:
            ...
            if datos.ruta_meli is None:
                ...
                conn.insert_bitacora_meli(data_supervisor.id_usuario, data_supervisor.ids_usuario, f'Ingreso de ambulancia id: {id_ambulancia}', float(data_supervisor.latitud), float(data_supervisor.longitud), data_supervisor.operacion)
                ...
                if existe_id_ruta == 0:
                    ...
                    conn.insert_bitacora_meli(data_supervisor.id_usuario, data_supervisor.ids_usuario, f'Ingreso de nueva citación', float(data_supervisor.latitud), float(data_supervisor.longitud), data_supervisor.operacion)

                else:
                    conn.update_datos_de_citacion_activa_FM(data_supervisor,datos)

                    conn.insert_bitacora_meli(data_supervisor.id_usuario, data_supervisor.ids_usuario, f'Actualización de citación : {datos.ruta_meli} ', float(data_supervisor.latitud), float(data_supervisor.longitud), data_supervisor.operacion)
