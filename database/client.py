        ...
        with self.conn.cursor() as cur:
            cur.execute(f"""
              ...
              select 	cp.codigo,
                        ...
                        array_agg(distinct(e.ubicacion)) 
                from rsv.catalogo_productos cp 
                JOIN LATERAL rsv.existencia(cp.codigo,1) AS inv ON true
                left join rsv.colores c on cp.color = c.id
                left join rsv.etiquetas e  on cp.codigo = e.codigo 
                where habilitado=true
                group by cp.codigo, c.nombre_color,cp.producto, inv.e_paquetes,inv.e_unidades,inv.total, cp.color
